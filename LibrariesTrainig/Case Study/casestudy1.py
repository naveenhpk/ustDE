...
# Create a scatter plot to visualize the relationship between exercise-induced angina
# (exang) and maximum heart rate (thalach).
# • Use Matplotlib to create the scatter plot.
# • Color the points based on the presence of heart disease (target).
# • Label the axes and provide a title.
# using Matplot
# scatter=plt.scatter(x=data['exang'],y=data['thalach'],c=data['target'],cmap='viridis',alpha=0.5,marker='^')
# plt.colorbar(scatter,label='target')
# sns.scatterplot(data=data,x='exang',y='thalach',hue='target')
plt.scatter(x=data['exang'].where(data['target']==1),y=data['thalach'].where(data['target']==1))
plt.scatter(x=data['exang'].where(data['target']==0),y=data['thalach'].where(data['target']==0))
plt.legend(["Heart Disease Present","Heart Disease Not Present"])
plt.xlabel('Exercise-induced angina')
plt.ylabel('Heart rate')
plt.title('Relationship B/W exercise-induced angina & maximum heart rate')
plt.show()